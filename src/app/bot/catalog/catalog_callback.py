    ...
    async def callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:  # После можно будет перекинуть в общие хендлеры
            await update.callback_query.edit_message_media(
                media=InputMedia(
                    media_type="photo", media=settings.CATALOG_IMAGE_ID, caption=text
                ),
                reply_markup=InlineKeyboardMarkup(
                    [
                        *[
                            catalog.to_button(int(offset), int(limit))
                            for catalog in catalogs
                        ],
                        *get_offset_limit_buttons(
                            int(offset), int(limit), count_catalogs
                        ),
                    ]
                ),
            )
...
def get_catalog_solo_callback():
    ...
    async def callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        await update.callback_query.edit_message_media(
            media=InputMedia(
                media_type="photo",
                media=catalog.file_unique_tg_id
                if catalog.file_unique_tg_id
            ...
            reply_markup=get_catalog_back_keyboard(
                ...
                int(offset),
                int(limit),
                return_to=update.callback_query.data,
            ),
        )
