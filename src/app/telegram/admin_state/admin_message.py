...
from .__addons import after_get_create_catalog_message_keyboard, start_update_catalogs_message_keyboard, get_back_to_catalogs_message_keyaboard


def get_enter_create_catalog_message():
...
def get_enter_update_catalogs_data_message():
    ...
    async def callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        user_message = update.message.text
        results: list[str] = re.findall(r"[0-9]+", user_message)
...
def get_update_calatogs_text_field_message():
    ...
    async def name_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        await update.message.reply_text(f"Обновлено имя: {update.message.text}", reply_markup=get_back_to_catalogs_message_keyaboard(catalog_id))
